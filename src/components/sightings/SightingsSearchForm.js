...
class SightingsSearchForm extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      date_from: query.date_from ? moment(query.date_from, MACHINE_DATE_FORMAT) : moment().subtract(1, 'months'),
      date_to: query.date_to ? moment(query.date_to, MACHINE_DATE_FORMAT) : moment(),
  ...
  componentDidMount() {
    ...
    dispatch(getSightings(PAGESIZE_SHOWALL));
  ...
  handleSubmit(e) {
    ...
    dispatch(push(`/sightings?${visibleQuery}`));
  }

  render() {
    return (
      <form className="SightingsSearchForm mb-3" onSubmit={ this.handleSubmit }>
...
export default compose(withRouter, connect(mapStateToProps))(SightingsSearchForm);
